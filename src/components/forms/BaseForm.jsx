...
import {
  EqualStringFields,
  NotEqualStringFields,
} from "./helpers/validation/multiple_elements/StringValidation";
import {
  EqualRegexStringField,
  EqualStringField,
  NotEqualRegexStringField,
  NotEqualStringField,
} from "./helpers/validation/single_element/StringValidation";
  ...
  useEffect(() => {
    ...
    props?.children?.map((element) => {
      ...
      if (element?.props?.name) {
        ...
        Object.keys(multiple_fields_validation_rules)
          .filter((key) => {
            return (
              multiple_fields_validation_rules[key].compare_to ===
              element?.props?.name
            );
          })
          .map((key) => {
            ...
            switch (field_operation) {
              case "=i": // TODO: = operator does not seems to be working with min, max, required fields
                if (stringElement)
                  validationBuilder = validationBuilder.EqualStringFields(
                    field1_value,
                    field_message ||
                      `${FieldToLabel(
                        element?.props?.name
              ...
              case "!=i":
                if (stringElement)
                  validationBuilder = validationBuilder.NotEqualStringFields(
                    field1_value,
                    field_message ||
                      `${FieldToLabel(
                        element?.props?.name
        ...
        Object.keys(individual_fields_validation_rules)
          .filter((key) => {
            return (
              individual_fields_validation_rules[key].field ===
              element?.props?.name
            );
          })
          .map((key) => {
            ...
            switch (field_operation) {
              case "=i":
                if (stringElement)
                  validationBuilder = validationBuilder.EqualStringField(
                    field_value,
                    field_message ||
                      `${FieldToLabel(
                        element?.props?.name
              ...
              case "=regex":
                if (stringElement)
                  validationBuilder = validationBuilder.EqualRegexStringField(
                    field_value,
                    field_message ||
                      `${FieldToLabel(
                        element?.props?.name
        ...
        if (element?.props?.min) {
          validationBuilder = validationBuilder.min(
            element?.props?.min,
            `${FieldToLabel(element?.props?.name)} should be minimum ${
              element?.props?.min
            } characters`
          );
        }
        if (element?.props?.max) {
          validationBuilder = validationBuilder.max(
            element?.props?.max,
            `${FieldToLabel(element?.props?.name)} should not be more than ${
              element?.props?.max
            } characters`
          );
        }
        if (element?.props?.required) {
          validationBuilder = validationBuilder.required(
            `${FieldToLabel(element?.props?.name)} is Required`
          );
        }
        queryBuilder = {
          ...queryBuilder,
      ...
      case "TextField":
        return React.cloneElement(element, {
          ...(showValidStatus
            ...
            : null),
          ...(showValidStatus && element.props?.validClass
            ? { validClass: element.props?.validClass }
            : null),
